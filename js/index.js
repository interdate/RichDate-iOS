...
var app = {
	...
	getUsers: function(){
		...
		console.log("PAGE NUMBER:" + app.pageNumber);
	...
	displayUsers: function(){
		...
		//if(app.currentPageId == 'users_list_page'){
			...
			if(app.response.users.itemsNumber == 0){
				app.usersContainer.append('<div class="center noResults">אין תוצאות</div>')
				return;
			}
			...
			if(app.responseItemsNumber == app.itemsPerPage){
				...
				//$('<br><br><br>').insertAfter(currentUserNode);
			}
...
//			alert(app.recentScrollPos + ' > ' + app.container.height() +' - ' +min);
			...
			var container = app.currentPageId == 'messenger_page' ? app.container : app.usersContainer;

			if(app.recentScrollPos >= container.height()-min){
				$(this).unbind("scroll");
